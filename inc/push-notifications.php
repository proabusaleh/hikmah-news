...
// ============================================
function hikmahnews_service_worker_script() {
    ?>
    <script>
        if ('serviceWorker' in navigator && 'PushManager' in window) {
            window.addEventListener('load', function() {
                navigator.serviceWorker.register('<?php echo esc_url(home_url('/hikmahnews-sw.js')); ?>')
                    .then(function(reg) {
...
add_action('wp_footer', 'hikmahnews_service_worker_script');

// ============================================
// 2. SERVE SERVICE WORKER FILE
// ============================================
function hikmahnews_serve_service_worker() {
    if ($_SERVER['REQUEST_URI'] !== '/hikmahnews-sw.js') return;

    header('Content-Type: application/javascript');
    header('Service-Worker-Allowed: /');

    $assets_uri = HIKMAHNEWS_URI . '/assets';
    ?>
    // Hikmah News Service Worker v1.0
    const CACHE_NAME = 'hikmahnews-v1';
    ...
    self.addEventListener('push', (event) => {
        ...
        const options = {
            ...
            image: data.image || '',
            tag: data.tag || 'hikmahnews-notification',
        ...
        event.waitUntil(
            self.registration.showNotification(data.title || 'Hikmah News', options)
        );
    });
...
add_action('init', 'hikmahnews_serve_service_worker');

// ============================================
// 3. NOTIFICATION PERMISSION PROMPT (Frontend)
// ============================================
function hikmahnews_notification_prompt() {
    ?>
...
add_action('wp_footer', 'hikmahnews_notification_prompt');

function hikmahnews_notification_prompt_script() {
    ...
    (function() {
        ...
        if (!localStorage.getItem("hikmahnews_notif_decision") &&
        ...
        if (allowBtn) {
            allowBtn.addEventListener("click", function() {
                Notification.requestPermission().then(function(perm) {
                    localStorage.setItem("hikmahnews_notif_decision", perm);
                        ...
                        if ("serviceWorker" in navigator) {
                            navigator.serviceWorker.ready.then(function(reg) {
                                reg.pushManager.subscribe({
                                    ...
                                    applicationServerKey: "' . esc_js(get_option('hikmahnews_vapid_public', '')) . '"
                                });
                            });
                        }
                    }
        ...
        if (dismissBtn) {
            dismissBtn.addEventListener("click", function() {
                localStorage.setItem("hikmahnews_notif_decision", "dismissed");
                prompt.style.display = "none";
            });
        }
    })();
...
add_action('wp_enqueue_scripts', 'hikmahnews_notification_prompt_script');

// ============================================
// 4. AUTO-NOTIFY ON BREAKING NEWS PUBLISH
// ============================================
function hikmahnews_send_push_on_breaking($post_id) {
    ...
    do_action('hikmahnews_send_push', $payload, $post_id);

    // Log notification
    $log = get_option('hikmahnews_push_log', []);
    $log[] = [
    ...
    update_option('hikmahnews_push_log', $log);
}
add_action('save_post', 'hikmahnews_send_push_on_breaking', 30);

// ============================================
// 5. ADMIN SETTINGS (VAPID Keys)
// ============================================
function hikmahnews_push_admin_settings() {
    add_options_page(
        'Push Notifications',
        '🔔 Push Notifications',
        'manage_options',
        'hikmahnews-push',
        'hikmahnews_push_settings_page'
    );
}
add_action('admin_menu', 'hikmahnews_push_admin_settings');

function hikmahnews_push_settings_page() {
    if (isset($_POST['hikmahnews_push_nonce']) && wp_verify_nonce($_POST['hikmahnews_push_nonce'], 'hikmahnews_push')) {
        update_option('hikmahnews_vapid_public', sanitize_text_field($_POST['vapid_public']));
        update_option('hikmahnews_vapid_private', sanitize_text_field($_POST['vapid_private']));
    ...
    <div class="wrap">
        ...
        <form method="POST">
            <?php wp_nonce_field('hikmahnews_push', 'hikmahnews_push_nonce'); ?>
            <table class="form-table">
                <tr>
                    <th>VAPID Public Key</th>
                    <td>
                        <input type="text" name="vapid_public" class="regular-text"
                               value="<?php echo esc_attr(get_option('hikmahnews_vapid_public')); ?>">
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="password" name="vapid_private" class="regular-text"
                               value="<?php echo esc_attr(get_option('hikmahnews_vapid_private')); ?>">
                    </td>
                </tr>
            </table>
